...
Codeine.prototype.runCommandSync = function (projectName, commandObject, sleep, times) {
  var scope = this;
  ...
  return this.runCommand(projectName, commandObject)
      .then(function (commandId) {
        ...
        function check(iteration) {
          log('runCommandSync() - waiting for command', commandId, 'to finish.' +
              ' Iteration:', iteration);
          return scope.getCommandStatusJson(projectName, commandId, false)
              .then(function (res) {
                  if (res.finished && (res.fail_list.length + res.success_list.length + (res.skip_list ? res.skip_list.length : 0)) === res.nodes_list.length) {
                      return _.map(res.fail_list, function (node) {
                          return node.name;
                      });
                  } else {
        ...
        return busywait(check, {sleepTime: sleep,maxChecks: times,
            ...
            .then(function (result) {
              var failedNodes = result.result;
              if (failedNodes.length > 0) {
                return Q.reject("Failed running command on " + failedNodes);
              }
