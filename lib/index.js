...
Codeine.prototype.saveProjectConfiguration = function(projectName, conf) {
    ...
    console.error('saveProjectConfiguration() - post data is ' + post_data);
    var commandReq = http.request(saveConfCommandOptions, function(res) {
        res.setEncoding('utf8');
        if (res.statusCode === 200) {
            console.error('saveProjectConfiguration() - Configuration was saved successfully');
        }
        else {
            console.error('saveProjectConfiguration() - Status Code =  ' + res.statusCode);
            deferred.reject();
        }
        res.on('data', function (chunk) {
            console.error('Response: ', chunk);
    ...
    commandReq.write(post_data);
    commandReq.on('error', function(e) {
        console.error('saveProjectConfiguration() - Error from server', e);
        deferred.reject(e);
    });
    commandReq.end();
    return deferred.promise;
